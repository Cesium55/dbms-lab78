...
def create_match_participants(matches, athletes, teams):
    for match in matches:
        ...
        if config['is_team_sport']:
            ...
            num_participants = random.randint(2, min(8, len(candidates)))
            is_team = False
        ...
        for i, participant in enumerate(selected):
            ...
            if response:
                part_type = "team" if is_team else "athlete"
                print(f"Added {part_type} {participant['id']} to match {match['id']}")
            else:
                print(f"Failed to add participant {participant['id']} to match {match['id']}")
